...
from wizwalker import HotkeyListener, Keycode, ModifierKeys, Orient, XYZ, utils
...
from src.utils import auto_potions, auto_potions_force_buy, collect_wisps_with_limit, get_window_from_path, index_with_str, is_free, is_visible_by_path, override_wiz_install_using_handle, read_webpage, to_world, try_task_coro  # , assign_pet_level
...
type_format_dict = {"char": "<c", "signed char": "<b", "unsigned char": "<B", "bool": "?", "short": "<h", "unsigned short": "<H", "int": "<i", "unsigned int": "<I", "long": "<l", "unsigned long": "<L", "long long": "<q", "unsigned long long": "<Q", "float": "<f", "double": "<d"}
...
async def main():
    ...
    async def tp_to_freecam_hotkey():
        ...
        if not await foreground_client.game_client.is_freecam():
            return
                    ...
                    # extra leeway on this allows clients to teleport more quickly to ensure that they arrive before the selection phase even starts
                        ...
                        if p.duel_circle_joinable and not p.in_solo_zone:
                            ...
                            for c in other_clients:
                                ...
                                if hitter_client is not None and hitter_client in c.title:
    ...
    def _send_hooked_clients_update():
        gui_send_queue.put(deimosgui.GUICommand(deimosgui.GUICommandType.UpdateHookedClients, _build_hooked_clients_info()))
                    ...
                    try:
                        ...
                        if cam is not None:
                            ...
                            if feet is not None and head is not None:
                                ...
                                gui_send_queue.put(deimosgui.GUICommand(deimosgui.GUICommandType.UpdateHighlightBox, (client.window_handle, x1, y1, x2, y2)))

                            elif feet is not None:
                                # Head behind camera - just show small box at feet
                                gui_send_queue.put(deimosgui.GUICommand(deimosgui.GUICommandType.UpdateHighlightBox, (client.window_handle, feet[0] - 30, feet[1] - 60, feet[0] + 30, feet[1])))
                ...
                while True:
                    try:
                        ...
                        if window and window is not False:
                            ...
                            gui_send_queue.put(deimosgui.GUICommand(deimosgui.GUICommandType.UpdateHighlightBox, (client.window_handle, rect.x1, rect.y1, rect.x2, rect.y2)))
                ...
                while True:
                    try:
                        ...
                        for entity in entities:
                            ...
                            display = (
                                f"{entity_name} (dist: {trunc(distance, 1)})"
                                f" - XYZ({trunc(entity_pos.x, 3)}, {trunc(entity_pos.y, 3)}, {trunc(entity_pos.z, 3)})"
                            )
                            entity_data.append({
                                "name": entity_name,
                                "x": entity_pos.x,
            ...
            if walker.clients and foreground_client:
                try:
                    ...
                    if dead:
                        ...
                        active_tasks = set()
                        task_vars = {"combat": combat_task, "dialogue": dialogue_task, "sigil": sigil_task, "questing": questing_task, "speed": speed_task, "bot": bot_task, "auto_pet": auto_pet_task}
                    ...
                    match com.com_type:
                        ...
                        case deimosgui.GUICommandType.HighlightEntity:
                            ...
                            if foreground_client and com.data:
                                highlight_task = asyncio.create_task(_highlight_entity_loop(foreground_client, com.data))
                        ...
                        case deimosgui.GUICommandType.HighlightUIWindow:
                            ...
                            if foreground_client and com.data:
                                highlight_task = asyncio.create_task(_highlight_ui_window_loop(foreground_client, com.data))
    ...
    def ban_thread():
        shake = discsdk.serialize_message(discsdk.Opcodes.Handshake, {"v": discsdk.rpc_version, "client_id": str(discsdk.app_id)})
    ...
    async def zone_check_loop():
        ...
        explicit_zone_blacklist = ["WizardCity/WC_Duel_Arena_New", "WizardCity/KT_Duel_Arena", "WizardCity/MB_Arena", "WizardCity/MS_Arena", "WizardCity/DS_Arena", "WizardCity/CL_Arena", "WizardCity/ZF_Arena", "WizardCity/AV_Arena", "WizardCity/AZ_Arena", "WizardCity/PA_Arena", "WizardCity/GH_Arena", "WizardCity/LM_Arena"]
    ...
    SNAPSHOT_TASK_NAMES = ["anti_afk_loop", "is_client_in_combat_loop", "entity_detect_combat_loop", "potion_usage_loop", "drop_logging_loop", "zone_check_loop", "anti_afk_questing_loop"]
    SNAPSHOT_TASK_FUNCS = {"anti_afk_loop": anti_afk_loop, "is_client_in_combat_loop": is_client_in_combat_loop, "entity_detect_combat_loop": entity_detect_combat_loop, "potion_usage_loop": potion_usage_loop, "drop_logging_loop": drop_logging_loop, "zone_check_loop": zone_check_loop, "anti_afk_questing_loop": anti_afk_questing_loop}
