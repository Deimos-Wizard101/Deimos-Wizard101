...
from PyQt6.QtWidgets import QFileDialog, QHBoxLayout, QPushButton, QTextEdit, QVBoxLayout, QWidget

from src.gui.commands import GUICommand, GUICommandType
from src.gui.helpers import add_recent, centered_label, repo_icon_btn, show_recent_menu
...
def build_flythrough_tab(ctx):
    ...
    header.addWidget(
        repo_icon_btn(ctx, ctx.svgs["readme"], ctx.tl("tooltip_wiki_flythroughs"), f"{ctx.wiki_base}/Flythroughs")
    )
    ...
    def flythrough_export():
        filepath, _ = QFileDialog.getSaveFileName(
            ctx.window, ctx.tl("export_flythrough"), "flythrough.txt", "Text Files (*.txt)"
        )
...
def build_bot_tab(ctx):
    ...
    toggle_btn, set_bot_running = _make_toggle_btn(
        ctx, ctx.tl("run_bot"), ctx.tl("kill_bot"), run_bot_callback, kill_bot_callback, "toggle_bot"
    )
...
def build_combat_tab(ctx):
    ...
    header.addWidget(centered_label(ctx.tl("advanced_warning")), 1)
    header.addWidget(
        repo_icon_btn(ctx, ctx.svgs["readme"], ctx.tl("tooltip_wiki_playstyles"), f"{ctx.wiki_base}/Playstyles")
    )
    layout.addLayout(header)
    ...
    def combat_export():
        filepath, _ = QFileDialog.getSaveFileName(
            ctx.window, ctx.tl("export_playstyle"), "playstyle.txt", "Text Files (*.txt)"
        )
        if filepath:
            try:
                with open(filepath, "w") as f:
    ...
    btn_row.addWidget(
        ctx.registry.action_icon_btn(ctx.svgs["refresh"], ctx.tl("set_playstyles"), set_playstyles_callback)
    )
    btn_row.addStretch()
    layout.addLayout(btn_row)
    return tab
