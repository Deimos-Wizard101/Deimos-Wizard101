...
from src.gui.helpers import centered_label, copy_callback, repo_icon_btn
...
def build_camera_tab(ctx):
    ...
    cam_header.addWidget(
        repo_icon_btn(ctx, ctx.svgs["readme"], ctx.tl("tooltip_wiki_camera"), f"{ctx.wiki_base}/Camera")
    )
        ...
        if default is not None:
            ...
            if default == "gid":
                ...
                reset_btn.clicked.connect(
                    lambda checked: ctx.send_queue.put(GUICommand(GUICommandType.PopulatePlayerGID))
                )
    ...
    def update_camera_callback():
        pos_vals = [
            cam_inputs["CamXInput"].text(),
            cam_inputs["CamYInput"].text(),
            cam_inputs["CamZInput"].text(),
            cam_inputs["CamYawInput"].text(),
            cam_inputs["CamRollInput"].text(),
            cam_inputs["CamPitchInput"].text(),
        ]
        ...
        dist_vals = [
            cam_inputs["CamDistanceInput"].text(),
            cam_inputs["CamMinInput"].text(),
            cam_inputs["CamMaxInput"].text(),
        ]
    ...
    utils_col.addWidget(
        ctx.registry.styled_btn(
            ctx.tl("copy_camera_position"), copy_callback(ctx.send_queue, GUIKeys.copy_camera_position)
        )
    )
    utils_col.addWidget(
        ctx.registry.styled_btn(
            ctx.tl("toggle_camera_collision"),
            lambda: ctx.send_queue.put(GUICommand(GUICommandType.ToggleOption, GUIKeys.toggle_camera_collision)),
        )
    )
