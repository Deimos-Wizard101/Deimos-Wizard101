...
def build_stats_tab(ctx):
    ...
    def _is_selection_valid():
        if not _swapped[0]:
            return (
                duel_circle.selected_caster() <= duel_circle._ally_count
                and duel_circle.selected_target() <= duel_circle._enemy_count
                and duel_circle._ally_count > 0
                and duel_circle._enemy_count > 0
            )

        else:
            return (
                duel_circle.selected_caster() <= duel_circle._enemy_count
                and duel_circle.selected_target() <= duel_circle._ally_count
                and duel_circle._ally_count > 0
                and duel_circle._enemy_count > 0
            )
