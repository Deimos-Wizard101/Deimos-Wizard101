...
def build_dev_utils_tab(ctx):
    ...
    dev_header.addWidget(
        repo_icon_btn(ctx, ctx.svgs["readme"], ctx.tl("tooltip_wiki_utilities"), f"{ctx.wiki_base}/Utilities")
    )
    ...
    _dev_fields = [
        ...
        (
            ctx.tl("select_pet_world"),
            "PetWorldInput",
            _dev_icons["paw"],
            "combo",
            {"items": pet_worlds, "default": "WizardCity"},
        ),
    ]
    ...
    def custom_tp_callback():
        ...
        tp_vals = [
            dev_inputs["XInput"].text(),
            dev_inputs["YInput"].text(),
            dev_inputs["ZInput"].text(),
            dev_inputs["YawInput"].text(),
        ]
