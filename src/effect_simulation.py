...
from wizwalker.memory.memory_objects.enums import HangingDisposition, MagicSchool, SpellEffects
...
hot_effect_types = {SpellEffects.heal_over_time}
...
def sim_outgoing_dmg_effects(
    cache: Cache, damage_type: int, damage: float, pierce: float
) -> Tuple[Cache, int, float, float]:
    ...
    for i, m_effects in enumerate(member_effects):
        ...
        for m_i, m_effect in enumerate(m_effects):
            if (
                m_effect["damage_type"] != MagicSchoolID.universal and m_effect["damage_type"] != damage_type
            ) or ids in used_ids:
                continue
...
def sim_outgoing_heal_effects(cache: Cache, heal_type: int, heal: float) -> Tuple[Cache, float]:
    ...
    for i, m_effects in enumerate(member_effects):
        ...
        for m_i, m_effect in enumerate(m_effects):
            if (
                m_effect["damage_type"] != MagicSchoolID.universal and m_effect["damage_type"] != heal_type
            ) or ids in used_ids:
                continue
        ...
        for m_i, m_effect in enumerate(m_effects):
            ...
            match m_effect["effect_type"]:
                ...
                case SpellEffects.absorb_damage:  # Spirit armor, frozen armor, etc
                    if damage < param:
                        cache_modify(
                            result_cache, param - damage, f"{hanging_effect_paths[i]}.{m_i}.effect_param"
                        )  # Modify absorb value if damage isn't enough to remove it
                        damage = 0
                        continue
        ...
        for m_i, m_effect in enumerate(m_effects):
            ...
            match m_effect["effect_type"]:
                ...
                case SpellEffects.absorb_heal:  # Heal absorb? Akin to heal flat resist?
                    if heal < param:
                        cache_modify(
                            result_cache, param - heal, f"{hanging_effect_paths[i]}.{m_i}.effect_param"
                        )  # Modify absorb value if heal isn't enough to remove it
                        heal = 0
                        continue
...
def calc_crit(
    crit_rating: float, block_rating: float, caster_level: int, target_level: int, is_pvp: bool = False
) -> Tuple[float]:
...
def sim_damage(
    duel: Cache, caster: Cache, target: Cache, effect: Cache, crit_threshold: float = 0.8
) -> Tuple[Cache, Cache, float]:
    ...
    dmg_percent_stat = cache_get(caster, "get_stats.dmg_bonus_percent")[damage_type_index] + cache_get(
        caster, "get_stats.dmg_bonus_percent_all"
    )
    ...
    damage += cache_get(caster, "get_stats.dmg_bonus_flat")[damage_type_index] + cache_get(
        caster, "get_stats.dmg_bonus_flat_all"
    )

    # Get and curve target resist
    resist = cache_get(target, "get_stats.dmg_reduce_percent")[damage_type_index] + cache_get(
        target, "get_stats.dmg_reduce_percent_all"
    )
    ...
    # Get pierce
    pierce = cache_get(caster, "get_stats.ap_bonus_percent")[damage_type_index] + cache_get(
        caster, "get_stats.ap_bonus_percent_all"
    )
    pierce = round(pierce, 2)
    ...
    crit_damage_multiplier, crit_chance, block_chance = calc_crit(
        caster_crit, target_block, caster_level, target_level, is_pvp
    )
    ...
    if caster["owner_id"] == target["owner_id"]:  # If we are the caster and target, ignore target and only use caster
        caster_result, damage_type, damage, pierce = sim_incoming_dmg_effects(
            caster_result, damage_type, damage, pierce
        )

    else:  # If the target is not ourselves, handle them normally
        target_result, damage_type, damage, pierce = sim_incoming_dmg_effects(
            target_result, damage_type, damage, pierce
        )

    damage_type_index = MagicSchoolIndex[damage_type].value

    # Flat resist
    damage -= cache_get(target, "get_stats.dmg_reduce_flat")[damage_type_index] + cache_get(
        target, "get_stats.dmg_reduce_flat_all"
    )
...
def get_multi_effects(
    effects: Iterable[Cache], valid_types: Set[SpellEffects], disposition: HangingDisposition = HangingDisposition.both
...
def sim_heal(
    ...
    heal_percent = cache_get(caster, "get_participant.get_stats.heal_bonus_percent")[heal_type_index] + cache_get(
        caster, "get_participant.get_stats.heal_bonus_percent_all"
    )
    heal *= 1 + heal_percent
    ...
    heal_inc_percent = cache_get(caster, "get_participant.get_stats.heal_inc_bonus_percent")[
        heal_type_index
    ] + cache_get(caster, "get_participant.get_stats.heal_inc_bonus_percent_all")
...
def sim_effect(duel: Cache, caster: Cache, target: Cache, effect: Cache) -> Cache:
    ...
    def _transfer_hanging_effects(
        origin: List[Cache],
        recipient: List[Cache],
        amount: int,
        valid_types: List[SpellEffects],
        disposition: HangingDisposition = HangingDisposition.both,
    ):
        _, effect_indices = get_multi_effects(origin, valid_types, disposition)
        for i in range(amount):
            if (
                len(effect_indices) < amount - i
            ):  # TODO: Verify if this actually works to check if we have enough effects
                break
    ...
    match effect["effect_type"]:
        ...
        case SpellEffects.damage_per_total_pip_power:  # Per ENEMY pips, like mana burn
            target_total_pip_value = (
                (target_total_spips * 2) + (target_pips["power_pips"] * 2) + target_pips["generic_pips"]
            )
        ...
        case SpellEffects.detonate_over_time:  # Detonate, Incindiate, Solomon Crane, some fire utility paths on R1-6
            # TODO: Simplify this
            dots = _pop_hanging_effects(
                target_effects[0], effect["effect_param"], dot_effect_types, effect["disposition"]
            )
            for dot in dots:
                target_result = sim_incoming_damage(
                    duel, target_result, dot["damage_type"], dot["effect_param"] * effect["heal_modifier"]
                )

        case SpellEffects.push_charm:
            _transfer_hanging_effects(
                caster_effects[0], target_effects[0], effect["effect_param"], charm_effect_types, effect["disposition"]
            )

        case SpellEffects.steal_charm:
            _transfer_hanging_effects(
                target_effects[0], caster_effects[0], effect["effect_param"], charm_effect_types, effect["disposition"]
            )

        case SpellEffects.push_ward:
            _transfer_hanging_effects(
                caster_effects[0], target_effects[0], effect["effect_param"], ward_effect_types, effect["disposition"]
            )

        case SpellEffects.steal_ward:
            _transfer_hanging_effects(
                target_effects[0], caster_effects[0], effect["effect_param"], ward_effect_types, effect["disposition"]
            )

        case SpellEffects.push_over_time:
            _transfer_hanging_effects(
                caster_effects[0], target_effects[0], effect["effect_param"], dot_effect_types, effect["disposition"]
            )

        case SpellEffects.steal_over_time:
            _transfer_hanging_effects(
                target_effects[0], caster_effects[0], effect["effect_param"], dot_effect_types, effect["disposition"]
            )
        ...
        case SpellEffects.swap_charm:
            caster_charms = _pop_hanging_effects(
                caster_effects[0], effect["effect_param"], charm_effect_types, effect["disposition"]
            )
            target_charms = _pop_hanging_effects(
                target_effects[0], effect["effect_param"], charm_effect_types, effect["disposition"]
            )
        ...
        case SpellEffects.swap_ward:
            caster_wards = _pop_hanging_effects(
                caster_effects[0], effect["effect_param"], ward_effect_types, effect["disposition"]
            )
            target_wards = _pop_hanging_effects(
                target_effects[0], effect["effect_param"], ward_effect_types, effect["disposition"]
            )
        ...
        case SpellEffects.swap_over_time:
            caster_dots = _pop_hanging_effects(
                caster_effects[0], effect["effect_param"], dot_effect_types, effect["disposition"]
            )
            target_dots = _pop_hanging_effects(
                target_effects[0], effect["effect_param"], dot_effect_types, effect["disposition"]
            )
...
pip_order = ["generic_pips", "power_pips"]
