...
def find_if_xyz_in_cube(xyz: XYZ, cube: list) -> bool:
    ...
    if (
        0 < multiply_xyz(v, i) < multiply_xyz(i, i)
        and 0 < multiply_xyz(v, j) < multiply_xyz(j, j)
        and 0 < multiply_xyz(v, k) < multiply_xyz(k, k)
    ):
        return True
...
def plotCube(ax, locs, color):
    ...
    ax.add_collection3d(Poly3DCollection(verts, facecolors=color, edgecolors=color, alpha=0.5))
...
def capsule_in_cube(capsule_center, capsule_radius, cube_vertices):
...
def capsule_in_sphere(capsule, sphere):
    ...
    distance = sqrt(
        (capsule_loc[0] - sphere_loc[0]) ** 2
        + (capsule_loc[1] - sphere_loc[1]) ** 2
        + (capsule_loc[2] - sphere_loc[2]) ** 2
    )
