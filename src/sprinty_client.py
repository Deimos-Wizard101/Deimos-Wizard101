...
from wizwalker import XYZ, Client, MemoryReadError
...
class SprintyClient:
    ...
    async def remove_excluded_entities_from(
        self, entities: List[DynamicClientObject], excluded_ids: Set[int] = None
    ...
    async def get_base_entities_with_name(self, name: str, excluded_ids: Set[int] = None):
        return await self.remove_excluded_entities_from(
            await self.client.get_base_entities_with_name(name), excluded_ids
        )

    async def get_base_entities_with_vague_name(
        self, name: str, excluded_ids: Set[int] = None
    ) -> List[DynamicClientObject]:
        async def _pred(e):
    ...
    async def find_safe_entities_from(
        self, entities: List[DynamicClientObject], safe_distance: float = 2000
    ) -> List[DynamicClientObject]:
        mob_positions = []
    ...
    async def find_closest_of_entities(
        self, entities: List[DynamicClientObject], only_safe: bool = False
    ...
    async def find_closest_by_predicate(
        self, pred: Callable, only_safe: bool = False, excluded_ids: Set[int] = None
    ) -> Optional[DynamicClientObject]:
        return await self.find_closest_of_entities(
            await self.get_base_entities_with_predicate(pred, excluded_ids), only_safe
        )

    async def find_closest_by_name(
        self, name: str, only_safe: bool = False, excluded_ids: Set[int] = None
    ) -> Optional[DynamicClientObject]:
        return await self.find_closest_of_entities(
            await self.get_base_entities_with_name(name, excluded_ids), only_safe
        )

    async def find_closest_by_vague_name(
        self, name: str, only_safe: bool = False, excluded_ids: Set[int] = None
    ) -> Optional[DynamicClientObject]:
        return await self.find_closest_of_entities(
            await self.get_base_entities_with_vague_name(name, excluded_ids), only_safe
        )

    async def find_closest_health_wisp(
        self, only_safe: bool = False, excluded_ids: Set[int] = None
    ) -> Optional[DynamicClientObject]:
        return await self.find_closest_of_entities(await self.get_health_wisps(excluded_ids), only_safe)

    async def find_closest_mana_wisp(
        self, only_safe: bool = False, excluded_ids: Set[int] = None
    ) -> Optional[DynamicClientObject]:
        return await self.find_closest_of_entities(await self.get_mana_wisps(excluded_ids), only_safe)

    async def find_closest_gold_wisp(
        self, only_safe: bool = False, excluded_ids: Set[int] = None
    ) -> Optional[DynamicClientObject]:
    ...
    async def tp_to_closest_by_vague_name(
        self, name: str, only_safe: bool = False, excluded_ids: Set[int] = None
    ) -> bool:
    ...
    async def use_potion_if_needed(
        self, health_percent: int = 20, mana_percent: int = 10, handle_hooks: bool = False
    ) -> bool:
        """
