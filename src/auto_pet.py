...
from wizwalker import XYZ, Client, HookAlreadyActivated, HookNotActive, HookNotReady, Keycode
...
async def nomnom(client: Client, ignore_pet_level_up: bool, only_play_dance_game: bool):
    ...
    while not finished_feeding:
        ...
        if total_energy >= energy_cost:
            ...
            if (
                only_play_dance_game or not await is_visible_by_path(client, skip_pet_game_button_path)
        ...
        while True:
            try:
                await safe_wait_for_zone_change(
                    client, name="WizardCity/WC_Streets/Interiors/WC_PET_Park", handle_hooks_if_needed=True
                )
                break
