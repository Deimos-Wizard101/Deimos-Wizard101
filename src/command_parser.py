...
from wizwalker import XYZ, Client, Keycode, Orient
...
from src.camera_utils import glide_to, orbit, point_to_xyz, rotating_glide_to
            ...
            match split_command[1].lower():
                    ...
                    match split_command[2]:
                        ...
                        case "quest" | "questpos" | "questposition":
                            await asyncio.gather(
                                *[p.teleport(await clients[0].quest_position.position()) for p in clients]
                            )
                ...
                case "waitforzonechange" | "wait_for_zone_change":
                    ...
                    if split_command[-2].lower() == "from":
                        await asyncio.gather(
                            *[wait_for_zone_change(client, current_zone=split_command[-1]) for client in clients]
                        )

                    # waits for zone to change to the provided zone name
                    elif split_command[-2].lower() == "to":
                        await asyncio.gather(
                            *[wait_for_zone_change(client, to_zone=split_command[-1]) for client in clients]
                        )
                ...
                case "usepotion":
                    ...
                    if len(split_command) > 3:
                        ...
                        await asyncio.gather(
                            *[
                                SprintyClient(p).use_potion_if_needed(
                                    health_percent=int(split_command[2]),
                                    mana_percent=int(split_command[3]),
                                    handle_hooks=True,
                                )
                                for p in clients
                            ]
                ...
                case "waitforwindow" | "waitforpath":
                    ...
                    if type(split_command[-1]) == str and split_command[-1].lower() == "completion":
                        ...
                        await asyncio.gather(
                            *[wait_for_visible_by_path(client, split_command[2], True) for client in clients]
                        )
                ...
                case "friendtp" | "friendteleport":
                    ...
                    if split_command[2] == "icon":
                        ...
                        await asyncio.gather(
                            *[teleport_to_friend_from_list_fish_icon_mouse_handler(client) for client in clients]
                        )
                        ...
                        await asyncio.gather(
                            *[teleport_to_friend_from_list_wizard_name_mouse_handler(client) for client in clients]
                        )

                case "entitytp" | "entityteleport":
                    # Teleports to a specific entity by vague name
                    await asyncio.gather(
                        *[SprintyClient(client).tp_to_closest_by_vague_name(split_command[2]) for client in clients]
                    )
...
async def parse_camera_command(camera: CameraController, command_str: str):
    ...
    match split_command[0].lower():
        case "glideto":
            if len(xyzs) >= 2:
                logger.debug(
                    f"Gliding freecam from {origin_pos} to {handle_index(xyzs)} while looking at {handle_index(xyzs, 1)} over {time} seconds"
                )

            else:
                logger.debug(
                    f"Gliding freecam from {origin_pos} to {handle_index(xyzs)} while orientated as {handle_index(orientations)} over {time} seconds"
                )
            ...
            await glide_to(
                ...
                origin_pos,
                handle_index(xyzs),
                handle_index(orientations, default=origin_orientation),
                time,
                handle_index(xyzs, 1),
            )
        ...
        case "rotatingglideto":
            logger.debug(
                f"Gliding freecam from {origin_pos} to {handle_index(xyzs)} while rotating {handle_index(orientations)} degrees over {time} seconds"
            )
        ...
        case "orbit":
            ...
            logger.debug(
                f"Orbiting freecam {degrees} degrees from {origin_pos} around {handle_index(xyzs)} over {time} seconds"
            )
