...
from wizwalker import XYZ, Client, HookAlreadyActivated, HookNotActive, Keycode, MemoryReadError, Rectangle
...
class Quester:
                ...
                if c_zone != leader_zone or maybe_solo_zone:
                    if await is_free(c) and not c.entity_detect_combat_status:
                        ...
                        async with c.mouse_handler:
                            ...
                            )  # icon_list=1, icon_index=self.current_leader_client.questing_friend_teleport_icon)
    ...
    async def heal_and_handle_potions(self):
        ...
        if any_client_needs_potions:
            ...
            await asyncio.gather(
                *[refill_potions(c, mark=False, recall=False, original_zone=await c.zone_name()) for c in self.clients]
            )
    ...
    async def determine_new_leader_and_followers(
        self, client_quests: dict, questing_clients: list[Client], follower_clients: list[Client]
    ...
    async def correct_dungeon_desync(self, follower_clients):
        ...
        await asyncio.gather(
            *[teleport_to_friend_from_list_wizard_leader_name_mouse_handler(c) for c in follower_clients]
        )
            ...
            for e in entities:
                try:
                    ...
                    display_name = await self.client.cache_handler.get_langcode_name(
                        display_name_code
                    )  # uses display name code to get display name text

                    match = fuzz.token_sort_ratio(
                        display_name.lower(), quest_item_list.lower()
                    )  # thefuzz check if display name matches quest item.
    ...
    async def collect_entity(self, e: DynamicClientObject) -> bool:
        ...
        for _ in range(2):  # try's to collect item twice if not safe
            ...
            if can_Teleport:
                ...
                await asyncio.sleep(0.2)  # waits 2 secs for the UI to load
    ...
    async def auto_pet_questing(self, questing_clients: list[Client], ignore_pet_level_up, play_dance_game):
        ...
        if auto_pet_on:
            ...
            if any_client_leveled_up:
                ...
                await asyncio.gather(
                    *[auto_pet(c, ignore_pet_level_up, play_dance_game, questing=True) for c in self.clients]
                )

    async def handle_zone_correction(
        self, maybe_solo_zone: bool, questing_friend_tp: bool, gear_switching_in_solo_zones: bool
    ):
    ...
    async def teleport_to_quest(self, hitting_client: str, follower_clients: list[Client]):
        ...
        if await is_free_leader_questing(self.current_leader_client):
            ...
            if "defeat" in leader_objective.lower():
                ...
                if await proxy_leader_client.zone_name() != zone_before_teleport or detected_dungeon:
                    ...
                    try:
                        await asyncio.gather(
                            *[
                                navmap_tp(
                                    client=c, xyz=leader_client_objective_xyz, leader_client=self.current_leader_client
                                )
                ...
                elif proxy_leader_is_questing and leader_objective != await self.get_truncated_quest_objectives(
                    proxy_leader_client
                ):
                    pass
            ...
            # furthermore staggered teleports can break certain quests in dungeons for certain clients
                ...
                await asyncio.gather(
                    *[
                        navmap_tp(p, leader_client_objective_xyz, leader_client=self.current_leader_client)
                        for p in self.clients
                    ]
                )
    ...
    async def handle_repeated_normal_quest_failures(
        self, last_leader_pid, last_leader_zone: str, iterations_since_last_quest_change: int
    ):
        ...
        if (
            ...
            if iterations_since_last_quest_change >= 5:
                ...
                await asyncio.gather(
                    *[p.teleport(XYZ(location.x + 500, location.y, location.z - 1500)) for p in self.clients]
                )
                await asyncio.sleep(2.0)
    ...
    async def bring_clients_to_same_location(self, questing_friend_tp: bool, gear_switching_in_solo_zones: bool):
        ...
        if questing_friend_tp:
            ...
            await self.zone_recorrect_friend_tp(
                maybe_solo_zone=maybe_solo_zone, gear_switching_in_solo_zones=gear_switching_in_solo_zones
            )

    async def initial_auto_pet(
        self, energy_info, questing_clients: list[Client], ignore_pet_level_up: bool, play_dance_game: bool
    ):
        auto_pet_on = False
        for p in self.clients:
            if p.auto_pet_status:
    ...
    async def auto_quest_leader(
        ...
        if questing_friend_tp or self.current_leader_client.auto_pet_status:
            ...
            energy_info = await asyncio.gather(
                *[return_wizard_energy_from_character_screen(c) for c in questing_clients]
            )
        ...
        while self.client.questing_status:
            ...
            follower_clients, client_quests = await self.determine_new_leader_and_followers(
            ...
            if await is_free_leader_questing(self.current_leader_client):
                ...
                if distance > 1:
                    ...
                    await self.handle_repeated_normal_quest_failures(
                        last_leader_pid, last_leader_zone, iterations_since_last_quest_change
                    )
                    ...
                    if distance < 1:
                        ...
                        incompatible_hardcoded_quests = {
                            "find submarine parts in the floating land": [
                                40,
                                XYZ(x=-17412.53515625, y=10505.794921875, z=-429.19927978515625),
                                XYZ(x=-17088.447265625, y=12284.244140625, z=-410.1100769042969),
                                XYZ(x=-21681.78125, y=11547.5966796875, z=-429.19927978515625),
                            ],
                            "collect sea cucumbers in pitch black lake": [
                                40,
                                XYZ(x=-8995.333984375, y=-830.6781616210938, z=-97.31965637207031),
                                XYZ(x=-9911.9404296875, y=823.7628173828125, z=-97.31971740722656),
                                XYZ(x=-11497.998046875, y=-1030.2618408203125, z=-97.32334899902344),
                                XYZ(x=-12099.0517578125, y=-4674.13623046875, z=-96.95536804199219),
                                XYZ(x=-14215.37109375, y=-4371.93798828125, z=-97.32661437988281),
                                XYZ(x=-14287.5556640625, y=-2835.4814453125, z=-97.3197021484375),
                            ],
                            "catch vonda fish in pitch black lake": [
                                3,
                                XYZ(x=-10030.9912109375, y=-7925.51953125, z=-347.3206787109375),
                            ],
                            "break mining equipment in tyrian gorge": [
                                25,
                                XYZ(x=-2529.12890625, y=-3015.832763671875, z=-906.33837890625),
                                XYZ(x=-3181.15234375, y=-6233.39306640625, z=-924.6146240234375),
                                XYZ(x=1495.8695068359375, y=-7369.19580078125, z=-905.3265380859375),
                                XYZ(x=-204.098876953125, y=-8468.421875, z=236.10702514648438),
                                XYZ(x=2508.4169921875, y=-4712.10302734375, z=-1232.338134765625),
                            ],
                            "steal barrel of kermes fire in tyrian gorge": [
                                35,
                                XYZ(x=1352.575927734375, y=-4787.16552734375, z=-1098.1168212890625),
                                XYZ(x=-1936.0185546875, y=-3954.15380859375, z=-1077.83837890625),
                                XYZ(x=-1820.606201171875, y=-6331.4541015625, z=-1077.849853515625),
                                XYZ(x=517.3740844726562, y=-7042.94287109375, z=-1077.84130859375),
                                XYZ(x=-4628.16650390625, y=-5603.2578125, z=-906.51318359375),
                                XYZ(x=-3216.014892578125, y=-5965.7412109375, z=-932.0715942382812),
                                XYZ(x=-1106.3753662109375, y=-3800.721923828125, z=-905.3256225585938),
                                XYZ(x=-451.9595031738281, y=-4547.75537109375, z=-905.325927734375),
                            ],
                        }
