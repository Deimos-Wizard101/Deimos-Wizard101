...
class Scope:
    def __init__(self, parent: Optional["Scope"], is_block: bool):
        ...
        # if all branching scopes agree on cleanup, do not clean up the same variables again
        self._cleaned_vars: set[Symbol] = set()

        # cleanup must not cross a block boundary as we do not have the notion of a moved variable
