...
from wizwalker import XYZ, AddressOutOfRange, Client, Keycode, MemoryReadError, Primitive
    ...
    async def _eval_expression(self, eval: Eval, client: Client):
        ...
        match kind:
            ...
            case EvalKind.any_player_list:
                return (
                    [c.title for c in self._any_player_client] if self._any_player_client else [self._clients[0].title]
                )
                    ...
                    match args[0]:
                        ...
                        case TeleportKind.minusteleport:
                            for client in clients:
                                ...
                                clientpos = await client.body.position()
                                newpos = XYZ(
                                    clientpos.x - minuspos.x, clientpos.y - minuspos.y, clientpos.z - minuspos.z
                                )
                                tg.create_task(client.teleport(newpos))
            ...
            case "tozone":
                ...
                async with asyncio.TaskGroup() as tg:
                    for client in clients:
                        ...
                        tg.create_task(
                            toZone([client], "/".join(zone_path) if isinstance(zone_path, list) else zone_path)
                        )
    ...
    async def exec_compound_deimos_call(self, command_entries):
        ...
        for entry in command_entries:
            ...
            instruction = Instruction(
                kind=InstructionKind.deimos_call, data=[player_selector, command_name, command_data]
            )
        ...
        match instruction.kind:
            ...
            case InstructionKind.deimos_call:
                ...
                deimos_call_instruction = Instruction(
                    kind=InstructionKind.deimos_call, data=[player_selector, command_name, data]
                )
